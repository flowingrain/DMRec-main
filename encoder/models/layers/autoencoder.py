...
# Optional import for kmeans clustering
try:
    from kmeans_pytorch import kmeans
    HAS_KMEANS = True
except ImportError:
    HAS_KMEANS = False
    print("Warning: kmeans_pytorch not available. Clustering features (n_cate > 1) will be disabled.")


class AutoEncoder(nn.Module):
    """
    Gaussian Diffusion for large-scale recommendation.
    Supports both single-category and multi-category (clustered) encoding/decoding.
    """
    def __init__(self, item_emb, n_cate, in_dims, out_dims, device, act_func, reparam=True, dropout=0.1):
        ...
        if n_cate == 1:  # no clustering
            ...
            if not HAS_KMEANS:
                raise ImportError("kmeans_pytorch is required for n_cate > 1. Install it with: pip install kmeans-pytorch")
                ...
                for i in range(n_cate):
                    ...
                    for d_in, d_out in zip(out_dims_temp[:-1], out_dims_temp[1:]):
                        ...
                        if self.act_func == 'relu':
                            ...
                            decoder_modules[i].append(nn.LeakyReLU())
                        else:
                            raise ValueError
...
def compute_loss(recon_x, x):
    """Multinomial log likelihood loss (as in MultVAE)"""
