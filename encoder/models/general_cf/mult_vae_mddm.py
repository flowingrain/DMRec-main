...
class mult_vae_MDDM(BaseModel):
    def __init__(self, data_handler):
        super(mult_vae_MDDM, self).__init__(data_handler)
    ...
    def cal_loss(self, user, batch_data):
        ...
        KLD_llm = - 0.5 * torch.mean(torch.sum(1 + torch.log(logvar.exp()/(logvar_llm.exp() + 10e-8) + 10e-8) -
                                           (mu - mu_llm).pow(2)/(logvar_llm.exp() + 10e-8) - logvar.exp()/(logvar_llm.exp() + 10e-8), dim=1))
