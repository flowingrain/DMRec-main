...
class RectifiedFMDMStrategy(object):
	...
	def __init__(self):
		...
		self.reflow_steps = hyper.get('reflow_steps', 2)  # Number of reflow iterations
		self.use_adaptive_sampling = hyper.get('use_adaptive_sampling', True)
	...
	def compute_flow_loss(self, mu_src, mu_llm, logvar_src, logvar_llm, epoch_idx=None, max_epoch=None):
		...
		if self.use_reflow and self.current_reflow_step > 0:
			...
			with torch.no_grad():
				# Transport z_0 using the current vector field
				z_1_refined = self.transport_sample(z_0, num_steps=self.reflow_steps)
			
			# Mix original z_1 and refined z_1
			# The mixing ratio can be adaptive based on reflow step
			# For now, use a fixed ratio that increases with reflow step
			mix_ratio = min(0.5, 0.3 * self.current_reflow_step)
